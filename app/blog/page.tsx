            ...
            {blogPosts.map((post) => (
              <Link href={`/blog/${post.slug}`} key={post.slug} style={{textDecoration:'none'}}>
                <div className="card" style={{padding:0,overflow:'hidden',cursor:'pointer',height:'100%',display:'flex',flexDirection:'column'}}>
                  <div style={{height:'200px',background:post.img,flexShrink:0}}></div>
                  <div style={{padding:'24px',display:'flex',flexDirection:'column',flex:1}}>
                    <span style={{color:'#94a3b8',fontSize:'13px',marginBottom:'8px',display:'block'}}>{post.date} &middot; {post.readTime}</span>
                    <h3 style={{fontSize:'20px',fontWeight:800,color:'#0f172a',marginBottom:'12px',lineHeight:1.4}}>{post.title}</h3>
                    <p style={{color:'#64748b',fontSize:'15px',marginBottom:'20px',flex:1}}>{post.excerpt}</p>
                    <span style={{color:'#7c3aed',fontSize:'14px',fontWeight:600}}>Read Article &rarr;</span>
                  </div>
                </div>
              </Link>
            ))}
